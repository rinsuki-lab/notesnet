...
CREATE INDEX idx_note_revisions_latest_written_at ON note_revisions (written_at DESC, inserted_at DESC, id DESC) WHERE next_revision_id IS NULL;
CREATE INDEX idx_note_revisions_latest_inserted_at ON note_revisions (inserted_at DESC, id DESC) WHERE next_revision_id IS NULL;
