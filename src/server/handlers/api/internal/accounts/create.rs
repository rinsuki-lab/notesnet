...
impl IntoResponse for CreateAccountError {
    fn into_response(self) -> axum::response::Response {
        match self {
            CreateAccountError::ServerError => (axum::http::StatusCode::INTERNAL_SERVER_ERROR, "Internal Server Error").into_response(),
            CreateAccountError::UserNameAlreadyUsed => (axum::http::StatusCode::CONFLICT, "This username is already in use").into_response(),
            CreateAccountError::InvalidUsername => (axum::http::StatusCode::UNPROCESSABLE_ENTITY, "Username must be 4-20 characters, lowercase alphanumeric only").into_response(),
            CreateAccountError::InvalidPassword => (axum::http::StatusCode::UNPROCESSABLE_ENTITY, "Password must be 16-128 characters").into_response(),
        }
    }
}
...
fn is_valid_username(username: &str) -> bool {
    ...
    if !username.chars().all(|b| b.is_ascii_lowercase() || b.is_ascii_digit()) {
        return false;
    }
    true
}
...
pub async fn create_account(
    ...
    let hashed_password = hasher.hash_password(body.password.as_bytes(), &salt)
        .map_err(|e| {
    ...
    sqlx::query!("INSERT INTO personas(id, account_id, name) VALUES ($1, $1, NULL)", res.id)
    ...
    sqlx::query!("INSERT INTO scopes(id, name, owner_account_id) VALUES ($1, 'Only for me', $1)", res.id)
        .execute(&mut *tx).await
        .map_err(|e| {
